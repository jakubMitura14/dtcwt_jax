...
from dtcwt_jax.numpyy.common import Pyramid
from dtcwt_jax.coeffs import biort as _biort, qshift as _qshift
from dtcwt_jax.defaults import DEFAULT_BIORT, DEFAULT_QSHIFT
from dtcwt_jax.utils import appropriate_complex_type_for, asfarray

from dtcwt_jax.numpyy.lowlevel import *
import einops
import pdb
...
class Transform3d(object):
    ...
    def forward(self, X, nlevels=3, include_scale=False, discard_level_1=False):
        ...
        # so we can see that level1 and other levels are diffrent
        # level is 0-indexed
...
def _level1_xfm_no_highpass(X, h0o, h1o, ext_mode):
    ...
    for f in range(X.shape[1]):
        # extract slice
        y = X[:, f, :].T
        out=out.at[:, f, :].set(colfilter(y, h0o).T)

    # Loop over 3rd dimension extracting 2D slice from first and 2nd dimensions
    for f in range(X.shape[2]):
        y = colfilter(out[:, :, f].T, h0o).T
        out=out.at[:, :, f].set(colfilter(y, h0o))
...
def _level1_ifm(Yl, Yh, g0o, g1o):
    ...
    if g0o.shape[0] % 2 == 0:
        return work[
            1 : (work.shape[0] >> 1), 1 : (work.shape[1] >> 1), 1 : (work.shape[2] >> 1)
        ]
    else:
...
def _level2_ifm(Yl, Yh, g0a, g0b, g1a, g1b, ext_mode, prev_level_size):
    ...
    for f in range(work.shape[1]):
        ...
        y = work[:, f, :].T
        work=work.at[:, f, :].set(
            einops.rearrange(colifilt(y[s2a, :], g0b, g0a) + colifilt(y[s2b, :], g1b, g1a), "a b -> b a")
        ).T
...
def c2cube(z):
    ...
    y=y.at[0::2, 0::2, 0::2].set(pr + qr + rr + sr)
    y=y.at[1::2, 0::2, 1::2].set(-pr - qr + rr + sr)
    y=y.at[1::2, 1::2, 0::2].set(-pr + qr + rr - sr)
    y=y.at[0::2, 1::2, 1::2].set(-pr + qr - rr + sr)

    y=y.at[0::2, 1::2, 0::2].set(pi - qi + ri - si)
    y=y.at[1::2, 1::2, 1::2].set(-pi + qi + ri - si)
    y=y.at[1::2, 0::2, 0::2].set(pi + qi - ri - si)
    y=y.at[0::2, 0::2, 1::2].set(pi + qi + ri + si)
